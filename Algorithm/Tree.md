...
- 모든 노드들이 **2개 이내의 서브트리**를 갖는 특별한 형태의 트리
...
- 높이가 h인 이진 트리가 가질 수 있는 노드의 **최소 개수는 (h+1)개가 되며, 최대 개수는 (2^(h+1)-1)개**가 된다.
- N개의 노드의 높이
    - 최악 : N
    - 최선 : logN
...
- **시간복잡도** : 반씩 탐색 범위를 줄인다. 최악 : 트리의 높이 만큼!, 최선 : O(logN)    
...
- 먼저 탐색 연산을 수행한다.
    ...
    - 리프 노드가 아니라면
        - 자식이 하나라면 부모와 연결시킨다.
        - 자식이 둘이라면 **왼쪽에서 가장 큰 수** 또는 **오른쪽에서 가장 작은 수**를 가져와 연결한다.
...
- 우선 마지막 자리에 넣어주고 자리를 바꿔주는 방식을 사용한다.
- 시간복잡도 : O(logN) (**완전이진트리**이기 때문에 보장되는 복잡도이다.)
    - 트리 높이 만큼 발생 
...
- **최단 거리** 탐색에 많이 쓰인다.
- "데이터 추가를 3000회 할 것 -> **정렬된 구조** 유지 필요"

- 우선 순위를 가진 항목들을 저장하는 큐
    - 우선 순위를 원소로 활용해서, 최대 힙을 활용한다.


## heapq

- `heap.heapify` : 리스트를 힙 자료구조로 변환
    - 아래쪽으로 갈수록, 노드 개수 많은데, 비교 교환 적음
    - 위로 갈수록, 노드 개수 적은데, 비교 교환 많아짐.
    - 따라서, O(N)
